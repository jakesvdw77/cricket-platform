...
- Business exceptions extend a fixed, documented set — `NotFoundException`, `ConflictException`, `ValidationException` — caught centrally by a `GlobalExceptionHandler` (`@RestControllerAdvice`). Exception → HTTP status matrix:
  ...
  Extend this table here before introducing a new exception type — don't invent a new one ad hoc in a controller.
