...
- **Refined after `027` and `028` both shipped, on direct user feedback:** `TeamForm`'s Section picker (the club-wide create flow) was a flat, alphabetically-ordered `Select` — genuinely ambiguous once a real club's tree had two branches reusing the same leaf name (e.g. "U13" under both Boys and Girls), indistinguishable in a flat list. Replaced with a new `SectionTreeSelect` component (a Select-shaped trigger that opens a real, expand/collapse `SectionTree` in a Popover, built on the new `@mui/x-tree-view` dependency — see `CLAUDE.md`'s tech stack table) — the closed field shows the full breadcrumb path once a value is picked, not just the ambiguous leaf name. `028-players.md`'s own Sections-tagging dialog got the identical fix in the same pass; both share the same `SectionTree`/`buildSectionTree` (extracted out of `025`'s `SectionTreeEditor`, which already had its own private tree-building logic doing the same job).
