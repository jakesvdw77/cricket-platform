...
- **`TeamCard.tsx`'s sponsor logos are now clickable**, opening the same Website/Email/Sponsor Contacts quick-view dialog `TeamDetailPage.tsx`'s own Sponsors grid opens (including the sponsor-contacts fetch above) — previously a static `Avatar` with only a hover tooltip, on the one card type the rest of this round's feedback was about (`TeamCard` is what `TeamDirectory.tsx`/`TeamList.tsx` render, the "main team card" the user meant).
