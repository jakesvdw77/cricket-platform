...
**Amended after merge (live testing against `059-record-card-click-to-view.md`'s shipped build, directed live by the user rather than through a separate spec cycle — same posture as this spec's own original round of live-testing fixes):**
- **`RecordIconButton` (shared with `056-club-profile-overview.md`'s `ClubOverviewPage.tsx`) now shows the record's name as a caption under the avatar**, not just in its `aria-label`/tooltip. The Contacts/Sponsors icon grids on both `TeamDetailPage.tsx` and `ClubOverviewPage.tsx` previously gave no way to tell who's who without opening each one's quick-view dialog.
- **`TeamDetailPage.tsx`'s Sponsors quick-view dialog now lists the sponsor's own named contacts** (`sponsorContactApi.listSponsorContacts`, fetched only while that dialog is open), alongside the existing Website/Email fields — omitted entirely when the sponsor has none, same pattern the rest of this spec's optional fields already use.
- **`SquadPlayerTile` no longer has a direct Edit action.** This screen is `TeamDetailPage` — a read-only "View" screen per `036-view-first-record-detail-screens.md`'s own posture — and a squad member shouldn't be directly editable from it. The tile's name remains the click-to-view stretched link into that player's own record (`059`), where Edit lives.
- **`TeamCard.tsx`'s sponsor logos are now clickable**, opening the same Website/Email/Sponsor Contacts quick-view dialog `TeamDetailPage.tsx`'s own Sponsors grid opens — previously a static `Avatar` with only a hover tooltip, on the one card type the rest of this round's feedback was about (`TeamCard` is what `TeamDirectory.tsx`/`TeamList.tsx` render, the "main team card" the user meant). Standards review caught that this dialog (plus its sponsor-contacts fetch) had been independently duplicated near-verbatim into both `TeamCard.tsx` and `TeamDetailPage.tsx` — well past `docs/standards/frontend.md`'s 70% reuse threshold — so it was extracted into a shared `ui/src/components/SponsorQuickViewDialog/` component before merge; both call sites now just pass `{ clubId, sponsor, onClose }`. Logged as a resolved instance under `docs/roadmap.md`'s broader `TeamCard`/`RecordCard` duplication entry.
- **`TeamCard.tsx`'s social links moved from a dedicated row near the bottom of the card (after sponsors) to the top-right corner, alongside the badge.** Real user feedback that they read as buried/an afterthought at the bottom; `SocialLinksRow` gained an optional `size="small"` prop (default unchanged at `"medium"` everywhere else it's used) to fit that compact corner.
- **`TeamFormPage.tsx`'s nested tabs were flattened.** This spec originally gave `TeamForm.tsx` its own inner Basic Info/Branding/Social Media tab bar, nested inside `TeamFormPage`'s own outer Details/Contacts/Sponsors/Squad tabs. Real user feedback found the two-level tab structure confusing. Branding and Social Media are now `TeamFormPage`'s own top-level tabs, in this exact order: **Details, Branding, Social Media, Contacts, Sponsors, Squad** — "Basic Info" no longer exists as a label; its fields are simply what "Details" shows. `TeamForm.tsx` still owns all of its field state in one always-mounted instance (a new `activeSection`/`hidden` prop pair lets the page control which fields show and whether the whole form is visible, without unmounting it when Contacts/Sponsors/Squad are showing) — switching tabs never loses an unsaved edit, verified directly with a dedicated regression test. `LeagueForm.tsx`/`LeagueFormPage.tsx` have the identical structurally-nested-tabs pattern this was copied from originally, but were deliberately left unchanged — this feedback was about the Team screen specifically, not a request to also flatten League's.
