...
- **Resolved post-ship, UI-only, no new migration or contract change:** the flag above was decided directly rather than deferred to a future Invoicing spec. Real-world use of the admin UI surfaced that burying day-to-day Subscription creation inside `Configuration` (meant for system-level setup — Products, System Settings — not a recurring billing workflow) was the wrong call. `Subscriptions` moved out to its own top-level `AdminHome.tsx` nav item, replacing the placeholder `Subscriptions & Invoices` entry; the route moved from `/admin/configuration/subscriptions*` to `/admin/billing*`. `Configuration`'s `Invoicing` placeholder card is untouched and still reserved for a future Invoicing spec to fill in — this change only resolves where the already-real `Subscriptions` screens live, not the Invoicing question itself.
