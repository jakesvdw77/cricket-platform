...
- **Multi-club subscriptions** — one `Subscription` covering several `Club`s under a single paid plan (e.g. an operator running multiple clubs). ADR-03's `Subscription.owner_id` stays a single `Club`/`Section`; a subscriber managing several clubs buys one subscription per club. Deferred deliberately: it's a real modeling change (a join table or an `Organization` concept above `Club`), and it raises a security-boundary question — not just a billing one — about whether a shared subscription implies shared admin/roster access across clubs, which would cut against this spec's Club-isolation guarantee. `League`/`LeagueAffiliation` already covers the narrower "cross-club stats visibility for a governing body" case (ADR-02) without needing shared billing; don't conflate the two when this comes up again. If the actual need turns out to be "one invoice for several clubs" rather than "shared access across clubs," that's a cheaper, purely billing-side addition — revisit only once a real subscriber asks for one or the other specifically.
