...
| `POST .../teams/{teamId}/seasons/{seasonId}/squad/{playerId}/add` | same | Adds a player to that season's squad. `404` if the player isn't a real player of this club, or `seasonId` belongs to a different club; `409` (`PlayerNotActiveClubMemberException extends ValidationException`... — see note below) if the player exists but is currently inactive; `409` if already in that season's squad |
