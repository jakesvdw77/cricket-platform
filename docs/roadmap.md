...
- **No real backend-verified route guard on `/manage`** for direct navigation by an already-authenticated user with no `CLUB_ADMIN` grant — `016` only wires the post-login *routing* decision, not a standing guard, matching `006`'s own original deferral.
...
- **A real backend-verified route guard on `/manage`** for an already-authenticated user with no `CLUB_ADMIN` grant navigating there directly — `016` only wires the post-login routing decision for a fresh login, not a standing guard; `006`'s own original deferral on Manager authorization still applies (`016` Non-goals).
