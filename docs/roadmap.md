...
| 015 | [Person Status & Role Assignment](specs/015-person-status-and-role-assignment.md) | Built, in review — added `Person.status` (`PENDING \| ACTIVE \| SUSPENDED`) and a real `RoleAssignment` entity/table (`CLUB_ADMIN`/`MANAGER`/`PLAYER`, `CLUB` scope only for now), and wired `AccessService.canAdministerClub` to a real lookup instead of `012`'s flat `platform_admin`-only stub. Its own Javadoc flagged that `RoleAssignment` branch as "correct but dormant" until something sets `Person.keycloakUserId` — that's `016`, below. |
| 016 | [Keycloak Account Provisioning](specs/016-keycloak-account-provisioning.md) | Draft — provisions a real Keycloak account (and sends a password-reset invite) for a Subscription's responsible `Person` (`014`), grants them a `CLUB_ADMIN` `RoleAssignment` (`015`), and activates the account on first login (JWT-email bridging, `PENDING` → `ACTIVE`, routes to `006`'s `/manage` shell). Makes `015`'s `RoleAssignment` branch reachable in production for the first time. |
| 020 | [Club Manager Access](specs/020-club-manager-access.md) | Built, reviewed, merged to `master` — new `/api/v1/manage/**` namespace + real `ManagerHome` route guard, first thing a `CLUB_ADMIN` can actually do with the grant `016` gives them. |
| 021 | [Club Contacts](specs/021-club-contacts.md) | Built, reviewed, merged to `master` — named-contacts list for a club, self-serviced via `020`'s `/manage` namespace, `/manage`-only (no `/platform` mirror — see its own Architecture note). |
| 022 | [Club Social Media](specs/022-club-social-media.md) | Built, reviewed, merged to `master` — social media links on `ClubProfile`, first of the three-spec Sponsors mini-epic (see `024`'s Rollout Notes); its `SocialLinksFields` editor is reused unmodified by `023`. |
| 023 | [Sponsors](specs/023-sponsors.md) | Built, reviewed, merged to `master` — a club's sponsor list (name, contact, branding, `022`'s social links), second of the mini-epic; sponsor contact *people* deliberately deferred to `024`. |
| 024 | [Sponsor Contacts](specs/024-sponsor-contacts.md) | Built, reviewed, merged to `master` (PR #31) — named contacts for a sponsor, a structural mirror of `021`'s `ClubContact` scoped two levels deep. Last of the mini-epic; see its own Rollout Notes for what this closes out. |
...
**Resolved by `016-keycloak-account-provisioning.md`:** a Subscription's responsible `Person` (`014`) now gets a real Keycloak account provisioned on Subscription creation (a new confidential `platform-provisioning` client, `POST /admin/realms/cricketlegend/users` + `execute-actions-email`), a `CLUB_ADMIN` `RoleAssignment` scoped to the owning `Club` (`015`'s tables, `RoleAssignmentRepository` used directly — no `RoleAssignmentService` yet), and first-login activation (`POST /api/v1/me/activate`, bridging the JWT to a `Person` by email, `PENDING` → `ACTIVE`) that routes the login to `006`'s existing `/manage` shell rather than `/admin`. This is also what makes `AccessService.canAdministerClub`'s `RoleAssignment` branch (`015`) reachable in production for the first time — `015`'s own "correct but dormant" Javadoc note is updated by `016` accordingly.

Left open by `016` itself, not resolved — see that spec's own Non-goals and Rollout Notes for the full reasoning behind each:
- **No retry mechanism or admin-visible indicator for a failed provisioning attempt.** A `Person` stuck in `PENDING` with `keycloakProvisionedAt: null` is discoverable only via a direct DB query or a Grafana log search (`013`) today.
- **No `PersonStatus`-aware authorization enforcement.** Neither `AccessService.canAdministerClub` nor the new first-login bridging consult `Person.status` — a `SUSPENDED` `Person` (nothing can set that status yet, see below) would still authenticate and still pass the `CLUB_ADMIN` check.
- ~~**No real backend-verified route guard on `/manage`**~~ — resolved by `020-club-manager-access.md`: `ManagerHome` now gates on `clubAdminClubIds` from `/api/v1/me/activate`, and the new `/api/v1/manage/**` endpoints carry a real `@PreAuthorize("@access.canAdministerClub(...)")` check, not just the post-login routing decision `016` originally wired.
- **Per-club branded Keycloak email templates** — plain default Keycloak email only this pass; real custom Keycloak theme work, deferred.
- `003-club-onboarding.md`'s still-unbuilt `Invitation` entity should very likely reuse `016`'s new `KeycloakProvisioningService` (create user + `execute-actions-email`) rather than rebuilding the same primitive when it's eventually spec'd — flagged in `016`'s own Rollout Notes.
...
- `015`'s `Person.status` enum — `PENDING`, reserved by `015`, is now actually set by `016`'s amended `PersonService.findOrCreatePerson` (every Subscription's responsible party starts `PENDING` until Keycloak activation). This future flow becomes a *second* caller of that already-real `PENDING` default, not the first to set it — its own job is the self-serve registration experience itself (OTP verification, Club configuration, claiming a subdomain), not inventing `PersonStatus` semantics from scratch.
...
`Section`, `Team`, and `ClubMembership` don't exist in code yet — only a minimal `Club` stub does (built for `004`'s public search). `RoleAssignment` itself is no longer blocked in the same way: `015-person-status-and-role-assignment.md` built it for real, for `CLUB` scope, and `016-keycloak-account-provisioning.md` is now a real, production consumer of it. Everything below is either blocked on `Section`/`Team` specifically, or is a gap `015`/`016` themselves flagged as still open:
...
- **`SECTION`/`TEAM`-scoped `RoleAssignment` resolution** — e.g. a Juniors-Section-scoped admin variant of `001`'s own worked example. `015`'s `ScopeType` enum reserves `SECTION`/`TEAM` as recognized values, but nothing creates, validates, or resolves a grant at either scope yet — blocked on `Section`/`Team` existing in code, the same gap `009` already identified for `Subscription.ownerType`. `016`'s own `CLUB_ADMIN` grant on Subscription creation is `CLUB`-scoped only, unchanged from `015`'s own limits.
- **A UI to grant/revoke `RoleAssignment` rows for a `Person`.** No admin screen exists — `015` built the data model and the `AccessService` read-side lookup only, and `016` (the first real writer of `RoleAssignment` rows) still only writes them as a side effect of Subscription creation, via `RoleAssignmentRepository` directly — no general-purpose grant/revoke screen either. Not blocked on `Section`/`Team` (a `CLUB`-scoped grant is buildable today, and now happens automatically for one specific case), just genuinely not built as a standalone UI yet.
- **Any `RoleAssignment`/administrative capability arising from a `Person` existing, for cases other than a Subscription's responsible party.** `PersonService.findOrCreatePerson` (`014`) itself still grants zero admin capability by itself — a `Person` being created or linked implies nothing about `RoleAssignment` on its own. `016` is what actually grants `CLUB_ADMIN` to a Subscription's responsible party specifically, as an explicit step in `SubscriptionServiceImpl.create()`, not a side effect of `Person` creation itself — any other future reason to grant a `RoleAssignment` (e.g. a manual admin grant, or `003`'s future `Invitation`-driven admin invites) still has no UI or generalized mechanism, per the bullet above.
- Everything `006` named and hasn't built: club onboarding's real screen (see `003` below), whitelisting, sections/teams/players management, results capture, communication, player profile/results/fixtures views.

**Resolved by `032-match-availability-polls.md`** for the club-admin + public-link case: a club admin can open/close an availability poll per side of a `Match` from a new Availability tab on `MatchFormPage`, and share a self-service response link — no player login required, by design. **Still open**: the login-gated `/player` side of this same feature (`006`'s "player profile/results/fixtures views" bullet above) — `032`'s `MatchAvailabilityPoll`/`PlayerAvailability` tables are deliberately player-identity-agnostic (`player_profile_id`-keyed, not session-derived) specifically so a future spec granting real player login/Keycloak provisioning (blocked on the same `RoleAssignmentRole.PLAYER`-ungranted gap `028`/`015` already flagged) can wire a genuine "My Availability" view against these same tables with zero schema rework.
...
**`020-club-manager-access.md`** (built) gave a club's own `CLUB_ADMIN` a real, backend-verified `/manage` surface for the first time — a new `/api/v1/manage/**` namespace (authenticated-only at the URL level, real `@PreAuthorize("@access.canAdministerClub(...)")` per endpoint) and a working Club Profile self-service screen. It's the foundation the two items below build directly on, per its own Rollout Notes.

**`021-club-contacts.md`** (built, merged) is the "Club Contacts" item below, now shipped: named people at a club — name, role, phone, email, one flagged primary — reusing `backend/src/main/java/com/cricketlegend/domain/Contact.java`/`ContactDto` (reserved for exactly this since `012`/`014`) and this project's `ListToolbar`/`RecordCard`/`RecordFormScreen` list pattern, on `020`'s `/manage`-only namespace (no `/platform` mirror — see that spec's own Architecture note for why). If a Club Contact entry ever needs to become login-capable, that upgrade path should resolve the contact's email through `014`'s `PersonService.findOrCreatePerson` rather than adding a schema-level link from `Contact` to `Person` — see `014`'s Rollout Notes for the full "bridge by email, not by FK" reasoning.

**"Sponsors"** (name, website, icon, banner, social links) is resolved — built across the three-spec mini-epic `022-club-social-media.md` (social links on `ClubProfile`) → `023-sponsors.md` (the `Sponsor` entity itself) → `024-sponsor-contacts.md` (named contact people for a sponsor, mirroring `021`'s `ClubContact`), all built, reviewed, and merged to `master`. Together they close out this item and everything named under the club-manager-self-service push `020` started (see `024`'s own Rollout Notes).
...
- **Whether a `SUSPENDED` `Person` should be blockable from being (re-)linked as a new Subscription's responsible party, or from completing Keycloak account provisioning/first-login activation.** `014`'s `PersonService.findOrCreatePerson` and `016`'s Keycloak provisioning/first-login bridging both link to or activate an existing `Person` by email regardless of `status`; flagged as a judgment call in `015`'s own Data Model Changes and restated by `016`'s own Non-goals. Still unresolved because nothing in this codebase can actually set `SUSPENDED` yet, and neither `AccessService.canAdministerClub` nor `016`'s first-login bridging consult `status` at all.
- **No retry mechanism or admin-visible indicator for a failed Keycloak provisioning attempt** (`016` Non-goals/Rollout Notes) — a `Person` left `PENDING` with `keycloakProvisionedAt: null` after a Keycloak outage is only discoverable via a direct DB query or a Grafana log search (`013`) today.
...
- **Production secrets management is entirely unresolved** — `016-keycloak-account-provisioning.md`'s `app.keycloak.admin.client-secret` is the first required (non-defaultable) secret this backend has ever needed (`${KEYCLOAK_ADMIN_CLIENT_SECRET:}`, no safe default). Local dev supplies it via a shell export or IDE run-config env var — fine for one developer's machine, not a real answer for a deployed server. No decision has been made yet on a production mechanism (platform-level env injection, a secrets manager, `.env` + `spring-dotenv`, etc.) — needs a real decision before this ships anywhere beyond local dev, and the same mechanism will likely need to cover any future secret this backend picks up, not just this one.
