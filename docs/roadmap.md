...
| 020 | [Club Manager Access](specs/020-club-manager-access.md) | Built, reviewed, merging to `master` — new `/api/v1/manage/**` namespace + real `ManagerHome` route guard, first thing a `CLUB_ADMIN` can actually do with the grant `016` gives them. |
| 021 | [Club Contacts](specs/021-club-contacts.md) | Spec'd, not yet built — named-contacts list for a club, self-serviced via `020`'s `/manage` namespace, `/manage`-only (no `/platform` mirror — see its own Architecture note). |
